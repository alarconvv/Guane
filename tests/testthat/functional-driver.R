...
#
# Use app.R in development and the installed entry point during package checks.
# GUANE_ASYNC=true exercises real mirai worker execution in both contexts.
...
guane_ft_app <- function(async = TRUE, workers = 1, name = 'guane', app_dir = guane_ft_root(), env = character()) {
 ...
 if (!file.exists(file.path(app_dir, 'app.R'))) {
  app_dir <- tempfile('guane-app-')
  dir.create(app_dir)
  withr::defer(unlink(app_dir, recursive = TRUE), envir = parent.frame())
  writeLines('guane::app_guane()', file.path(app_dir, 'app.R'))
 }
