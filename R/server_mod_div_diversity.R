server_mod_div_diversity <- function(input,output,session,data) {
 ...
 shiny::observeEvent(list(data$state$tree,input$dd_models,input$dd_missing,input$dd_res,input$dd_cond,input$dd_btorph,input$dd_starts,input$dd_free,input$dd_tol,input$dd_tolint,input$dd_maxiter,input$dd_cycles,input$dd_optimizer,input$dd_method,input$dd_threshold,input$dd_change,input$dd_verbose),{dd_task$discard();dd_unc_task$discard();dd_diag_task$discard();result(NULL);status('Inputs changed. Run diversification models to update results.')},ignoreNULL=FALSE)
 dd_task<-guane_task(function(r){result(r);shiny::updateSelectInput(session,'dd_model',choices=names(r$fits));status(if(all(r$comparison$Converged))'Analysis completed.' else 'Inspect convergence and backend messages.');data$record('Diversity-dependent diversification analysis completed.')},status,status)
 dd_unc_task<-guane_task(function(r){if(is.null(result()))return();result(r);status('Analysis completed.')},status,status)
 shiny::observeEvent(input$dd_run,{
  result(NULL)
  tryCatch(dd_task$run(guane_dd_fit,c(list(tree=data$state$tree),options())),error=function(e)status(conditionMessage(e)))
 })
 shiny::observeEvent(input$dd_factor,{dd_diag_task$discard();r<-result();if(!is.null(r)){r$diagnostics<-NULL;result(r)}},ignoreInit=TRUE)
 dd_diag_task<-guane_task(function(r){if(is.null(result()))return();result(r);status('Analysis completed.')},status,status)
 shiny::observeEvent(input$dd_diagnose,{shiny::req(result());tryCatch(dd_diag_task$run(guane_dd_diagnose,list(result(),value('factor',2))),error=function(e)status(conditionMessage(e)))})
 ...
 shiny::observeEvent(list(input$dd_unc_model,input$dd_unc_parameter,input$dd_lower,input$dd_upper,input$dd_points,input$dd_level,input$dd_profiles,input$dd_robustness,input$dd_start_factors,input$dd_factor,input$dd_bootstrap,input$dd_nsim,input$dd_bootseed,input$dd_seconds),{dd_unc_task$discard();r<-result();if(!is.null(r)){r$uncertainty<-NULL;result(r)}},ignoreInit=TRUE)
 shiny::observeEvent(input$dd_unc_run,{shiny::req(result());tryCatch(dd_unc_task$run(guane_dd_uncertainty,c(list(result=result()),uncertainty_options())),error=function(e)status(conditionMessage(e)))})
 ...
 for(field in c('profile','interval','robustness','bootstrap','bootstrap_intervals'))local({key<-field;output[[paste0('dd_',key,'_csv')]]<-shiny::downloadHandler(paste0('guane-dd-',key,'.csv'),function(file){shiny::req(result()$uncertainty[[key]]);guane_write_csv(result()$uncertainty[[key]],file,row.names=FALSE)})})
 ...
 output$dd_csv<-shiny::downloadHandler('guane-diversity-estimates.csv',function(file)guane_write_csv(estimates(),file,row.names=FALSE))
 for(field in c('comparison','diagnostics'))local({key<-field;output[[paste0('dd_',key,'_csv')]]<-shiny::downloadHandler(paste0('guane-diversity-',key,'.csv'),function(file){shiny::req(result()[[key]]);guane_write_csv(result()[[key]],file,row.names=FALSE)})})
