...
guane_hidden_script <- function(result,settings=guane_hidden_settings()) {
 ...
 c('# GUane hidden-state analysis: saved fits and editable plots. Requires ape, hisse and data.table for refitting.',paste('# Original hisse version:',result$version),guane_script_helpers(functions),guane_r_assignment('result',result),guane_r_assignment('settings',settings),'# Optional refit: result <- do.call(guane_hidden_fit,result$inputs)','opened <- grDevices::dev.cur() == 1L','if(opened) grDevices::pdf("guane-hidden.pdf",width=settings$width,height=settings$height)','guane_hidden_plot(result,settings)','if(opened) grDevices::dev.off()','print(result$comparison)','print(result$warnings)','sessionInfo()')
}
