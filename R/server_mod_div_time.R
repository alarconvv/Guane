server_mod_div_time <- function(input,output,session,data) {
 ...
 shiny::observeEvent(list(data$state$tree,options(),time_options()),{time_task$discard();time_unc_task$discard();if(!is.null(time_result()))status('Time-model settings changed. Run time-varying comparison again.');time_result(NULL)},ignoreNULL=FALSE)
 time_task<-guane_task(function(r){time_result(r);shiny::updateSelectInput(session,'time_graph',selected='time_rates');status('Time-model analysis completed. Inspect diagnostics and bounds.')},status,status)
 time_unc_task<-guane_task(function(r){if(is.null(time_result()))return();time_result(r);status('Time uncertainty completed. Review limits, failures and robustness.')},status,status)
 shiny::observeEvent(input$time_run,{
  ...
  tryCatch({
   ...
   time_task$run(guane_rates_tv_fit,c(list(tree=data$state$tree),shared,time_options()))
 ...
 shiny::observeEvent(time_unc_options(),{
  time_unc_task$discard();r<-time_result();if(!is.null(r)){r$uncertainty<-r$uncertainty_inputs<-NULL;time_result(r);status('Time uncertainty settings changed. Run time uncertainty again.')}
 },ignoreInit=TRUE)
 shiny::observeEvent(input$time_unc_run,{
  shiny::req(time_result())
  tryCatch({
   time_unc_task$run(guane_rates_tv_uncertainty,c(list(result=time_result()),time_unc_options()))
  },error=function(e)status(conditionMessage(e)))
 })
 ...
 output$time_unc_csv<-shiny::downloadHandler('guane-time-profile.csv',function(file){shiny::req(time_result()$uncertainty$profile);guane_write_csv(cbind(Model=time_result()$uncertainty$model,time_result()$uncertainty$profile$intervals),file,row.names=FALSE)})
 output$time_profile_curves_csv<-shiny::downloadHandler('guane-time-profile-curves.csv',function(file){shiny::req(time_result()$uncertainty$profile);guane_write_csv(cbind(Model=time_result()$uncertainty$model,time_result()$uncertainty$profile$curves),file,row.names=FALSE)})
 output$time_boot_csv<-shiny::downloadHandler('guane-time-bootstrap-refits.csv',function(file){shiny::req(time_result()$uncertainty$bootstrap);guane_write_csv(cbind(Model=time_result()$uncertainty$model,time_result()$uncertainty$bootstrap$replicates),file,row.names=FALSE)})
 output$time_bands_csv<-shiny::downloadHandler('guane-time-bootstrap-bands.csv',function(file){shiny::req(time_result()$uncertainty$bootstrap$bands);guane_write_csv(cbind(Model=time_result()$uncertainty$model,time_result()$uncertainty$bootstrap$bands),file,row.names=FALSE)})
 output$time_robust_csv<-shiny::downloadHandler('guane-time-robustness.csv',function(file){shiny::req(time_result()$uncertainty$robustness);guane_write_csv(cbind(Model=time_result()$uncertainty$model,time_result()$uncertainty$robustness),file,row.names=FALSE)})
 ...
 output$time_csv<-shiny::downloadHandler('guane-time-comparison.csv',function(file){shiny::req(time_result());guane_write_csv(time_result()$comparison,file,row.names=FALSE)})
 output$time_curves_csv<-shiny::downloadHandler('guane-time-curves.csv',function(file){shiny::req(time_result());guane_write_csv(time_result()$curves,file,row.names=FALSE)})
 output$time_attempts_csv<-shiny::downloadHandler('guane-time-optimizer.csv',function(file){shiny::req(time_result());guane_write_csv(time_result()$attempts,file,row.names=FALSE)})
