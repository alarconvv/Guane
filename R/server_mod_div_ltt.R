server_mod_div_ltt <- function(input,output,session,data) {
 ...
 output$ltt_code<-shiny::renderText({if(is.null(result()))return(paste('# guane_ltt_run(trees, include_stem, tol)',paste('# include_stem =',isTRUE(input$ltt_stem),'| tol =',{tol<-suppressWarnings(as.numeric(value('ltt_tol',1e-6))[1]);if(length(tol)==1&&is.finite(tol))format(tol,digits=6) else 'invalid'}),sep='\n'));paste(code(),collapse='\n')})
 output$ltt_csv<-shiny::downloadHandler('guane-ltt.csv',function(file){shiny::req(result());guane_write_csv(result(),file,row.names=FALSE)})
